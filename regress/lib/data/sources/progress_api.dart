...
class ProgressAPI {
  ...
  Future<ResultDart<http.Response, String>> _get(String token, String path, [Map<String, String>? headers]) async {
  ...
  Future<ResultDart<String, Unit>> fetchUserImage(String userUuid, String jwtToken) =>
      _get(jwtToken, "image/$userUuid").fold((res) => res.body.toSuccess(), (error) => unit.toFailure());
  ...
  Future<ResultDart<String, Unit>> fetchUniversityLogo(String jwtToken, String establishmentId) =>
      _get(jwtToken, "logoEtablissement/$establishmentId")
          .fold((res) => res.body.toSuccess(), (error) => unit.toFailure());

  // https://progres.mesrs.dz/api/infos/bac/{uuid}/dias
  Future<ResultDart<List<StudentBacInfoResponseV2Entity>, String>> fetchStudentData(String jwtToken, String studentUuid) =>
      _get(jwtToken, "bac/$studentUuid/dias").fold((res) {
        ...
        if (studentDataDecoded is List) {
          result = studentDataDecoded.map((e) => StudentBacInfoResponseV2Entity.fromJson(e)).toList(growable: false);
        } else {
  ...
  Future<ResultDart<List<StudentGroupEntity>, String>> fetchStudentGroups(String jwt, String studentId) =>
  ...
  Future<ResultDart<SessionToken, String>> login(String registrationNumber, String password) async {
